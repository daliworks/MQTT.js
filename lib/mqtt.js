...
module.exports.createSecureClient = function(port, host, opts) {
  ...
  if (opts.keyPath || opts.certPath || opts.caPath || opts.pfxPath) {
    tls_opts.key = opts.keyPath && fs.readFileSync(opts.keyPath);
    tls_opts.cert = opts.certPath && fs.readFileSync(opts.certPath);
    tls_opts.ca = opts.caPath && fs.readFileSync(opts.caPath);
    tls_opts.pfx = opts.pfxPath && fs.readFileSync(opts.pfxPath);
    tls_opts.rejectUnauthorized = opts.rejectUnauthorized || false;
    if (opts.servername) {
      tls_opts.servername = opts.servername;
    }
    if (opts.passphrase) {
      tls_opts.passphrase = opts.passphrase;
    }
  }

  builder = function() {
    var conn = tls.connect(port, host, tls_opts, function() {
    ...
    conn.on('secureConnect', function() {
      if (tls_opts.rejectUnauthorized && !conn.authorized) {
